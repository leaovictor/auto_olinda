...
import '../../../features/appointments/domain/availability.dart';
...
class BookingRepository {
  ...
  Stream<List<ServicePackage>> getServicesStream() {
    return _firestore.tenantCol(tenantId, 'services').snapshots().map((snapshot) {
  ...
  Future<void> createService(ServicePackage service) async {
    ...
    data.remove('id');
    await _firestore.tenantCol(tenantId, 'services').add(data);
  }

  Future<void> updateService(ServicePackage service) async {
    final data = service.toJson();
    data.remove('id');
    await _firestore.tenantCol(tenantId, 'services').doc(service.id).update(data);
  }

  Future<void> deleteService(String serviceId) async {
    await _firestore.tenantCol(tenantId, 'services').doc(serviceId).delete();
  }

  Future<ServicePackage?> getService(String serviceId) async {
    final doc = await _firestore.tenantCol(tenantId, 'services').doc(serviceId).get();
  ...
  Future<DocumentReference> createVehicle(Vehicle vehicle, String userId) {
    ...
    data.remove('id');
    return _firestore.tenantCol(tenantId, 'vehicles').add({
      ...data,
      'userId': userId,
      'tenantId': tenantId,
  ...
  Future<Vehicle?> getVehicle(String vehicleId) async {
    final doc = await _firestore.tenantCol(tenantId, 'vehicles').doc(vehicleId).get();
    if (!doc.exists) return null;
  ...
  Future<bool> checkAvailability(DateTime startTime) async {
    ...
    final availabilityDoc = await _firestore
        .tenantCol(tenantId, 'availability')
        .doc(dateStr)
        .get();
  ...
  Stream<Booking> getBookingStream(String bookingId) {
    return _firestore.tenantCol(tenantId, 'appointments').doc(bookingId).snapshots().map(
      (doc) {
        if (!doc.exists) {
  ...
  Future<void> updatePaymentStatus(
    ...
    return _firestore.tenantCol(tenantId, 'appointments').doc(bookingId).update(updates);
  }
  ...
  Future<void> markAsRated(
    ...
    return _firestore.tenantCol(tenantId, 'appointments').doc(bookingId).update({
      'isRated': true,
      'rating': rating,
