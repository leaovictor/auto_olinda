...
content = content.replace(
  'const { priceId, successUrl, cancelUrl, vehicleId, linkedPlate, planName, bonusWashes } = request.data;',
...
content = content.replace(
  /metadata:\s*\{\s*userId,\s*vehicleId,\s*linkedPlate,\s*planName,\s*bonusWashes:\s*bonusWashes\s*\?\s*bonusWashes\.toString\(\)\s*:\s*"0",\s*\}/g,
  'metadata: { userId, vehicleId, linkedPlate, planName, bonusWashes: bonusWashes ? bonusWashes.toString() : "0", tenantId }'
);

content = content.replace(
  /metadata:\s*\{\s*userId,\s*vehicleId,\s*linkedPlate,\s*planName\s*\}/g,
  'metadata: { userId, vehicleId, linkedPlate, planName, tenantId }'
);
