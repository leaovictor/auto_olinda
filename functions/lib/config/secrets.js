...
exports.getSecretOrDefault = getSecretOrDefault;
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
function getSecret(secret, envKey) {
    if (isEmulator()) {
        const value = process.env[envKey];
        if (!value) {
            throw new Error(`[Emulator] Secret "${envKey}" is not set. ` +
    ...
    try {
        ...
        throw new Error(`[Production] Failed to resolve secret "${envKey}": ${err.message}`);
    }
}
/**
 ...
 */
function getSecretOrDefault(secret, envKey, defaultValue) {
    try {
